---
title: "Understanding Child Marriage Through In Vietnam's Socio-Demographic Prism"
author: "Holly Duong"
date: "2024-02-29"
output:
...
```{r message=FALSE, warning=FALSE}
# import dataset
female <- read_csv("/Users/hollyduong/Desktop/DA 401/ChildMarriageInVietnam/Raw Data/wm.csv")
```
...
### Create New Variable `Access to Media`
...
### Data Filtering (IMPORTANT: keeping only data of married demographics)
...
### Missing Data Imputation (For Approach 1)
...
### `Age_first_marriage` Distribution

```{r}
# Histogram for 'Age at First Marriage'
afm_hist <- ggplot(imputed_df, aes(x = age_first_marriage)) +
...
```{r}
#ggsave("age_first_marriage.png", plot = afm_hist, width = 8, height = 6)
```
...
mapvn <- ggplot(data = vietnam_map_data) +
  ...
  labs(title = "Map of Vietnam: \nRegional Contribution to National Child Marriage Rates") +
  theme_void() +
...
```{r}
# Add a new column for total percentages
counts_df$total_percentage <- counts_df$married_u18 + counts_df$married_u16

p <- ggplot(counts_df, aes(x = reorder(factor(region), -total_percentage))) +
...
# 5. Approach 1: Logistic Regression Analysis with Data Imputation
...
## Models Comparison (Odd Ratios and 95% Confidence Intervals)
...
# 6. Approach 2: Logistic Regression Analysis with No Data Imputation
*After careful consideration regarding data reliability, this project will proceed with Approach 2.*
...
# 8. Models Comparison (Odd Ratios and 95% Confidence Intervals)
